...
def _has_deadlocked(target_id, *, seen_ids, candidate_ids, blocking_on):
    ...
    for tid in candidate_ids:
        ...
        if _has_deadlocked(
            target_id, seen_ids=seen_ids, candidate_ids=edges, blocking_on=blocking_on
        ):
            return True
...
def _module_repr_from_spec(spec):
    ...
    if spec.origin is None:
        ...
        if loader is None:
            return f"<module {name!r}>"
        elif _bootstrap_external is not None and isinstance(
            loader, _bootstrap_external.NamespaceLoader
        ):
