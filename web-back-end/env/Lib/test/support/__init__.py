...
def _is_gui_available():
    ...
    if sys.platform.startswith("win") and platform.win32_is_iot():
        ...
        try:
            rc = subprocess.run(
                ["launchctl", "managername"], capture_output=True, check=True
            )
...
has_fork_support = hasattr(os, "fork") and not (
    is_emscripten or is_wasi or is_apple_mobile
)
...
has_subprocess_support = not (is_emscripten or is_wasi or is_apple_mobile)
...
def iter_slot_wrappers(cls):
    ...
    for name in dir(cls):
        ...
        try:
            ...
            assert name in ("__annotate__", "__annotations__", "__abstractmethods__"), (
                cls,
                name,
            )
